...
def get_term_listing(
    ...
    try:
        return TermListing.Schema().loads(answer), answer
...
def get_sense_definition(
    ...
    try:
        return TermSenseDefinition.Schema().loads(answer), answer
    except Exception as e:
        raise ValueError(answer) from e
...
def get_synonym_comparison(
    ...
    return reblock(completion(SYNONYM_PREFX, "\n".join(query)))
...
def query(
    ...
    for sense in term_listing.senses:
        ...
        print()
        print(sense_info.long_definition)
